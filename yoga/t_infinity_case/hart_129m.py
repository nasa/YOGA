...
weights = [129176139,3538003,3538003,3538003,3538003]
domain_names = ["fuselage",
        "blade-1",
        "blade-2",
        "blade-3",
        "blade-4"]
mesh_filenames = ["/hpnobackup1/cdruyor/big-ole-chopper/hartii_sting_v4_refined3.merged_embed.b8.ugrid",
            "/hpnobackup1/cdruyor/big-ole-chopper/blade_scaled_corrected_pitch.b8.ugrid",
            "/hpnobackup1/cdruyor/big-ole-chopper/blade_scaled_corrected_pitch.b8.ugrid",
            "/hpnobackup1/cdruyor/big-ole-chopper/blade_scaled_corrected_pitch.b8.ugrid",
            "/hpnobackup1/cdruyor/big-ole-chopper/blade_scaled_corrected_pitch.b8.ugrid"]
...
mesh = infinity.import_mesh("RefinePlugins", mesh_filename, comm)
...
for step in range(10):
    if(0 == global_comm.rank()):
        print("Step "+str(step))
    if("blade" in domain_name and step > 0):
        motion = infinity.Motion()
        motion.addRotation(axis_begin,axis_end,1.0)
        mesh = infinity.moveMesh(mesh,motion)
