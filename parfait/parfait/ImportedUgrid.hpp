...
inline Parfait::Point<double> Parfait::ImportedUgrid::getPoint(int nodeId) const { return getNode(nodeId); }
...
inline int Parfait::ImportedUgrid::numberOfNodes() const { return nnodes; }

inline int Parfait::ImportedUgrid::numberOfCells() const { return ncells; }

inline int Parfait::ImportedUgrid::numberOfBoundaryFaces() const { return nfaces; }

inline int Parfait::ImportedUgrid::numberOfFacesInCell(int id) const {
    if (id < cellMap[0]) return 4;
    if (id < cellMap[2]) return 5;
    if (id < cellMap[3]) return 6;
    throw std::logic_error("couldn't find cell");
}

inline int Parfait::ImportedUgrid::numberOfNodesInCell(int id) const {
    if (id < cellMap[0]) return 4;
    if (id < cellMap[1]) return 5;
    if (id < cellMap[2]) return 6;
    if (id < cellMap[3]) return 8;
    throw std::logic_error("couldn't find cell");
}

inline int Parfait::ImportedUgrid::numberOfNodesInBoundaryFace(int id) const {
    if (id < faceMap[0]) return 3;
    if (id < faceMap[1]) return 4;
    throw std::logic_error("couldn't find cell");
}

inline int Parfait::ImportedUgrid::numberOfNodesInCellFace(int cellId, int faceId) const {
    if (cellId < cellMap[0]) return 3;
    if (cellId < cellMap[1]) {
        if (faceId == 0)
            return 4;
        else
...
inline std::vector<int> Parfait::ImportedUgrid::getNodesInCell(int cellId) const {
    ...
    for (int i = 0; i < nvertices; i++) cell[i] = cell_list[nvertices * id + i];
    return cell;
}
...
inline int Parfait::ImportedUgrid::getBoundaryTag(int id) const {
    ...
    PARFAIT_THROW("You asked for a boundary tag for an invalid face id: " + std::to_string(id))
}
