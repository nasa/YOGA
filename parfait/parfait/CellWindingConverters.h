...
class CGNSToVtk {
  public:
    ...
    template <typename T>
    static void convertHexa_20(T* cell) {
        std::swap(cell[12], cell[16]);
        std::swap(cell[13], cell[17]);
        std::swap(cell[14], cell[18]);
        std::swap(cell[15], cell[19]);
    }
};
...
class CGNSToFun3D {
  public:
    ...
    template <typename T>
    static void convertTetEdges(T* tet_edges) {
        T tmp = tet_edges[3];
    ...
    template <typename T>
    static void convertPyramidEdges(T* pyramid_edges) {
        std::array<T, 8> tmp_pyramid;
        for (int i = 0; i < 8; i++) tmp_pyramid[i] = pyramid_edges[i];
    ...
    template <typename T>
    static void convertPrismEdges(T* prism_edges) {
        std::array<T, 9> tmp_prism;
        for (int i = 0; i < 9; i++) tmp_prism[i] = prism_edges[i];
    ...
    template <typename T>
    static void convertHexEdges(T* hex_edges) {
        std::array<T, 12> tmp_hex;
        for (int i = 0; i < 12; i++) tmp_hex[i] = hex_edges[i];
