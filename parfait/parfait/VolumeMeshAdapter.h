...
class VolumeMeshAdapter : public VolumeMeshInterface {
  ...
  public:
    ...
    virtual int numberOfNodes() const override { return numberOfNodes_m(); }

    virtual int numberOfCells() const override { return numberOfCells_m(); }

    virtual std::vector<int> getNodesInCell(int cellId) const override { return getNodesInCell_m(cellId); }

    virtual Point<double> getPoint(int nodeId) const override { return getPoint_m(nodeId); }
};
}