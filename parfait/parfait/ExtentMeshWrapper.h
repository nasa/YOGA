...
class ExtentMeshWrapper {
  public:
    ...
    Point<double> getNode(int i) {
        ...
        int vertex = i % 8;
        auto p = e.lo;
        if (1 == vertex or 2 == vertex or 5 == vertex or 6 == vertex) p[0] += e.getLength_X();
        if (2 == vertex or 3 == vertex or 6 == vertex or 7 == vertex) p[1] += e.getLength_Y();
        if (3 < vertex) p[2] += e.getLength_Z();
        return p;
    }
