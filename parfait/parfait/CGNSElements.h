...
namespace CGNS {
    namespace Triangle {
        constexpr int numberOfNodes() { return 3; }
        constexpr int numberOfEdges() { return 3; }
        static constexpr std::array<std::array<int, 2>, 3> edge_to_node = {{{0, 1}, {1, 2}, {2, 0}}};

        template <typename Tri>
        Parfait::Point<double> computeCenter(const Tri& tri) {
            return (tri[0] + tri[1] + tri[2]) / 3.0;
        }

        template <typename Tri>
        std::array<Parfait::Point<double>, 3> computeEdgeCenters(const Tri& tri) {
            std::array<Parfait::Point<double>, 3> edge_centers;
            edge_centers[0] = 0.5 * (tri[0] + tri[1]);
            edge_centers[1] = 0.5 * (tri[1] + tri[2]);
            edge_centers[2] = 0.5 * (tri[2] + tri[0]);
            return edge_centers;
        }
    }
    namespace Quad {
        constexpr int numberOfNodes() { return 4; }
        constexpr int numberOfEdges() { return 4; }
        static constexpr std::array<std::array<int, 2>, 4> edge_to_node = {{{0, 1}, {1, 2}, {2, 3}, {3, 0}}};
    }
    namespace Tet {
        constexpr int numberOfNodes() { return 4; }
        constexpr int numberOfEdges() { return 6; }
        static constexpr std::array<std::array<int, 2>, 6> edge_to_node = {
            {{0, 1}, {1, 2}, {2, 0}, {0, 3}, {1, 3}, {2, 3}}};

        static constexpr std::array<std::array<int, 2>, 6> edge_to_face = {
            {{0, 1}, {0, 2}, {0, 3}, {1, 3}, {2, 1}, {3, 2}}};
    ...
    namespace Pyramid {
        constexpr int numberOfEdges() { return 8; }
        ...
        constexpr static std::array<std::array<int, 2>, 8> edge_to_face{
            {{0, 1}, {0, 2}, {0, 3}, {0, 4}, {1, 4}, {2, 1}, {3, 2}, {4, 3}}};
        template <typename Pyramid>
        Parfait::Point<double> computeCenter(const Pyramid& pyramid) {
            return 0.2 * (pyramid[0] + pyramid[1] + pyramid[2] + pyramid[3] + pyramid[4]);
        }
    ...
    namespace Prism {
        constexpr int numberOfEdges() { return 9; }
    ...
    namespace Hex {
        constexpr int numberOfEdges() { return 12; }
        ...
        Parfait::Point<double> computeCenter(const Hex& hex) {
            return 1.0 / 8.0 * (hex[0] + hex[1] + hex[2] + hex[3] + hex[4] + hex[5] + hex[6] + hex[7]);
        }

        template <typename Hex>
        std::array<Parfait::Point<double>, 12> computeEdgeCenters(const Hex& hex) {
            std::array<Parfait::Point<double>, 12> edge_centers;
            for (int i = 0; i < 12; i++) edge_centers[i] = 0.5 * (hex[edge_to_node[i][0]] + hex[edge_to_node[i][1]]);
            return edge_centers;
        }
        template <typename Hex>
