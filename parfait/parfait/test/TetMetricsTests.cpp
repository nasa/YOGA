...
TEST_CASE("Tet Center") {
    std::array<Parfait::Point<double>, 4> tet;
    tet[0] = {0, 0, 0};
    tet[1] = {1, 0, 0};
    tet[2] = {0, 1, 0};
    tet[3] = {0, 0, 1};
    auto center = std::accumulate(tet.begin(), tet.end(), Parfait::Point<double>{}) / 4.0;
    REQUIRE(center.approxEqual(Parfait::CGNS::Tet::computeCenter(tet)));
}
