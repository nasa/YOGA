...
TEST_CASE("distributed to contiguous, serial") {
    ...
    auto distributed_to_contiguous =
        Parfait::GraphOrderings::buildDistributedToContiguous(MPI_COMM_WORLD, local_to_global, do_own);
    REQUIRE(distributed_to_contiguous.size() == 14);
    for (size_t i = 0; i < distributed_to_contiguous.size(); i++) {
...
TEST_CASE("Sparse to ordinal") {
    auto mp = MessagePasser(MPI_COMM_WORLD);
    if (mp.NumberOfProcesses() != 1) return;
    std::vector<std::vector<int>> graph = {{0, 1}, {0, 1, 2}, {1, 2, 3}, {2, 3, 4}, {3, 4}};
    std::vector<long> l2g = {10, 11, 22, 33, 44};
    std::vector<bool> do_own = {true, true, true, true, true};
    auto sparse_to_ordinal_global =
        Parfait::GraphOrderings::buildDistributedToContiguous(mp.getCommunicator(), l2g, do_own);
    REQUIRE(sparse_to_ordinal_global.size() == 5);
    REQUIRE(sparse_to_ordinal_global.at(10) == 0);
    REQUIRE(sparse_to_ordinal_global.at(11) == 1);
    REQUIRE(sparse_to_ordinal_global.at(22) == 2);
    REQUIRE(sparse_to_ordinal_global.at(33) == 3);
    REQUIRE(sparse_to_ordinal_global.at(44) == 4);
}
