...
TEST_CASE("CGNSToAflr, tetsDontChange") {
    ...
    auto original = tet;
    Parfait::CGNSToAflr::convertTet(tet.data());
...
TEST_CASE("CGNSToAflr, pyramidsDoChange") {
    ...
    Parfait::CGNSToAflr::convertPyramid(pyramid.data());
...
TEST_CASE("CGNSToAflr, prismsDontChange") {
    ...
    auto original = prism;
    Parfait::CGNSToAflr::convertPrism(prism.data());

    for (int i = 0; i < 6; i++) REQUIRE(original[i] == prism[i]);
}

TEST_CASE("CGNSToAflr, hexsDontChange") {
    std::array<int, 8> hex{0, 1, 2, 3, 4, 5, 6, 7};
    auto original = hex;
    Parfait::CGNSToAflr::convertHex(hex.data());

    for (int i = 0; i < 8; i++) REQUIRE(original[i] == hex[i]);
}
