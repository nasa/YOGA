...
class Block {
  public:
    T& operator()(int row, int col) { return data[BlockRow * row + col]; }
    const T& operator()(int row, int col) const { return data[BlockRow * row + col]; }
...
class MatrixBlockSparse {
  public:
    MatrixBlockSparse(const std::vector<long>& global_rows,
                      const std::vector<std::vector<long>>& global_cols_of_local_rows)
        : data(countEntries(global_cols_of_local_rows)) {
        local_row_start.resize(global_rows.size() + 1);
        local_row_start[0] = 0;
        global_column_index.resize(data.size());
        size_t index = 0;
        for (size_t r = 0; r < global_rows.size(); r++) {
        ...
        for (size_t local_row = 0; local_row < global_rows.size(); ++local_row) {
    ...
    size_t countEntries(const std::vector<std::vector<long>>& global_cols_of_local_rows) const {
        ...
        for (auto& row : global_cols_of_local_rows) total_entries += row.size();
    ...
    Block<T, BlockRow, BlockCol>& operator()(long row, long col) {
        ...
        for (long index = local_row_start[local_row]; index < local_row_start[local_row + 1]; index++) {
            if (global_column_index[index] == col) return data[index];
        }
...
TEST_CASE("Sparse matrix exists") {
    ...
    std::vector<std::vector<long>> global_colums_in_local_row = {{7, 8}, {8, 9}, {9, 10}, {10, 11}};
