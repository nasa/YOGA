...
TEST_CASE("Pyramid Center") {
    ...
    pyramid[3] = {0, 1, 0};
    pyramid[4] = {0, 0, 1};
    auto center = std::accumulate(pyramid.begin(), pyramid.end(), Parfait::Point<double>{}) / 5.0;
    REQUIRE(center.approxEqual(Parfait::CGNS::Pyramid::computeCenter(pyramid)));
}
