...
inline Parfait::CartBlock::CartBlock(int ncells_x, int ncells_y, int ncells_z)
    : Extent({0, 0, 0}, {1, 1, 1}), kx(ncells_x), ky(ncells_y), kz(ncells_z) {
    number_of_cells = kx * ky * kz;
}

inline Parfait::CartBlock::CartBlock(
    const Point<double>& min_xyz, const Point<double>& max_xyz, int ncells_x, int ncells_y, int ncells_z)
    : Extent(min_xyz, max_xyz), kx(ncells_x), ky(ncells_y), kz(ncells_z) {
    number_of_cells = kx * ky * kz;
}

inline Parfait::CartBlock::CartBlock(const Extent<double>& extent, int ncells_x, int ncells_y, int ncells_z)
    : Extent(extent.lo, extent.hi), kx(ncells_x), ky(ncells_y), kz(ncells_z) {
    number_of_cells = kx * ky * kz;
}
...
inline int Parfait::CartBlock::getIdOfContainingCell(const double point[3]) const {
    if (not intersects(Point<double>(point))) throw std::domain_error("Point not contained by any cell in block");
    double hx, hy, hz;
...
inline std::vector<int> Parfait::CartBlock::getCellIdsInExtent(const Extent<double>& b) const {
    ...
    for (int i = slice.lo[0]; i < slice.hi[0]; i++)
        for (int j = slice.lo[1]; j < slice.hi[1]; j++)
            for (int k = slice.lo[2]; k < slice.hi[2]; k++) cell_ids.push_back(convert_ijk_ToCellId(i, j, k));
    return cell_ids;
}
...
inline Parfait::Extent<int> Parfait::CartBlock::getRangeOfOverlappingCells(const Extent<double>& search_extent) const {
    Extent<int> id_range{{0, 0, 0}, {0, 0, 0}};
    if (!intersects(search_extent)) return id_range;
    Point<double> dx{get_dx(), get_dy(), get_dz()};
    Point<int> nx{kx, ky, kz};

    for (int i = 0; i < 3; i++) {
        id_range.lo[i] = (search_extent.lo[i] - lo[i]) / dx[i];
        id_range.hi[i] = (search_extent.hi[i] - lo[i]) / dx[i] + 1;
        id_range.lo[i] = bound(id_range.lo[i], 0, nx[i] - 1);
        id_range.hi[i] = bound(id_range.hi[i], 0, nx[i]);
    }

    return id_range;
}

inline Parfait::Extent<int> Parfait::CartBlock::getRangeOfOverlappingCells_dep(
    const Extent<double>& search_extent) const {
    if (!intersects(search_extent)) return Extent<int>({0, 0, 0}, {0, 0, 0});

    Extent<double> fitted_search_extent = search_extent;
    double tol = 1.0e-15;
    Point<double> fuzz{tol, tol, tol};
    fitted_search_extent.lo -= fuzz;
    fitted_search_extent.hi += fuzz;
    for (int i = 0; i < 3; i++) {
...
inline Parfait::Extent<double> Parfait::CartBlock::createExtentFromCell(int cell_id) const {
    ...
    b[1] = a[1] + dy;
    b[2] = a[2] + dz;
    if (i == numberOfCells_X() - 1) b[0] = hi[0];
    if (j == numberOfCells_Y() - 1) b[1] = hi[1];
    if (k == numberOfCells_Z() - 1) b[2] = hi[2];
    Extent e(a, b);
    return e;
}
