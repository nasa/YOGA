...
class GraphOrderings {
  public:
    inline static std::map<long, long> buildDistributedToContiguous(MPI_Comm comm,
        ...
        auto sync_pattern = Parfait::SyncPattern(mp, have, need);
        ...
        auto local_to_petsc_global = assignOwnedContiguousGlobal(do_own, contiguous_row_start, local_to_owned_compact);
    ...
    inline static std::vector<int> buildLocalToCompactOwned(const std::vector<bool>& do_own) {
        ...
        for (int local = 0; local < int(do_own.size()); local++) {
            if (do_own[local]) local_to_compact[local] = next_compact++;
        }
    ...
    inline static std::tuple<std::vector<long>, std::vector<long>> buildHaveNeed(
        const std::vector<long>& local_to_global, const std::vector<bool>& do_own_local) {
        std::vector<long> have, need;
        for (int local = 0; local < int(local_to_global.size()); local++) {
            if (do_own_local[local])
                have.push_back(local_to_global[local]);
            else
                need.push_back(local_to_global[local]);
        }
        return {have, need};
    };
    ...
    inline static std::map<long, long> buildGlobalToGlobalMap(const std::vector<long>& local_to_global_distributed,
