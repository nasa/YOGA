...
inline double elapsedTimeInSeconds(Clock::time_point t0, Clock::time_point t1) {
    ...
    return ms.count() / ((double)1000 * 1000);
}
...
inline std::string readableElapsedTimeAsString(Clock::time_point t0, Clock::time_point t1) {
    ...
    long number_of_milliseconds =
        ms.count() - 3600000 * number_of_hours - 60000 * number_of_minutes - 1000 * number_of_seconds;

    std::string out;
    if (0 < number_of_hours) out.append(std::to_string(number_of_hours) + " hours ");
    if (0 < number_of_minutes) out.append(std::to_string(number_of_minutes) + " minutes ");
