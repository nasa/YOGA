...
inline void MatrixVectorMultiply(const double* A, const double* v, double* x, int nrows, int ncols) {
    ...
    for (int row = 0; row < nrows; row++)
        for (int col = 0; col < ncols; col++) x[row] += A[row * ncols + col] * v[col];
}

inline void MatrixMatrixMultiply(
    const double* A, const double* B, int anrows, int ancols, int bnrows, int bncols, double* O) {
    assert(ancols == bnrows);
    for (int i = 0; i < anrows * bncols; i++) O[i] = 0.0;
