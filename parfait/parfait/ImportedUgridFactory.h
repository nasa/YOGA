...
class ImportedUgridFactory {
  public:
    ...
    static void createBoundaryConditionsFromTags(std::string filename,
...
inline ImportedUgrid ImportedUgridFactory::readUgrid(std::string filename) { return readUgrid(filename, false); }

inline void ImportedUgridFactory::createBoundaryConditionsFromTags(std::string filename,
                                                                   std::vector<int>& triangleTags,
                                                                   std::vector<int>& quadTags,
                                                                   std::vector<int>& triangleBoundaryConditions,
    ...
    if (std::string::npos != position) mapbcFile.resize(position);
    ...
    for (int tag : triangleTags) triangleBoundaryConditions.push_back(reader.boundaryCondition(tag));
    for (int tag : quadTags) quadBoundaryConditions.push_back(reader.boundaryCondition(tag));
}
...
inline ImportedUgrid ImportedUgridFactory::readUgrid(std::string filename, bool isBigEndian) {
    ...
    UgridReader reader(filename, isBigEndian);

    nodes = reader.readNodes();
    triangles = reader.readTriangles();
    quads = reader.readQuads();
    tets = reader.readTets();
    pyramids = reader.readPyramids();
    prisms = reader.readPrisms();
    hexs = reader.readHexs();
    triangleTags = reader.readTriangleBoundaryTags();
    quadTags = reader.readQuadBoundaryTags();

    createBoundaryConditionsFromTags(
        filename, triangleTags, quadTags, triangleBoundaryConditions, quadBoundaryConditions);
...
inline ImportedUgrid ImportedUgridFactory::readUgridAscii(std::string filename) {
    ...
    fscanf(fp, "%d %d %d %d %d %d %d\n", &numNodes, &numTris, &numQuads, &numTets, &numPyramids, &numPrisms, &numHexs);
    ...
    for (int nodeId = 0; nodeId < numNodes; nodeId++) {
        fscanf(fp, "%lf %lf %lf", &nodes[3 * nodeId + 0], &nodes[3 * nodeId + 1], &nodes[3 * nodeId + 2]);
    }
