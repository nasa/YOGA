...
inline SparseMatrix::SparseMatrix(SparseMatrix&& m) : num_rows(m.num_rows), num_non_zeroes(m.num_non_zeroes) {
    ia = m.ia;
    ja = m.ja;
    data = m.data;
    m.ia = nullptr;
...
inline SparseMatrix::SparseMatrix(int num_rows_in, int num_non_zeroes_in)
    ...
    if (data == nullptr) throw std::logic_error("Could not allocate space for sparse matrix");
}
inline SparseMatrix SparseMatrix::fromCompressedRowStorage(int num_rows, int num_non_zeros, int* ia_in, int* ja_in) {
    ...
    if (m.ia == nullptr or m.ja == nullptr) throw std::logic_error("Could not allocate space for csr format");
...
inline double& SparseMatrix::operator()(int row, int col) {
    ...
    for (long index = ia[row]; index < ia[row + 1]; index++) {
        if (ja[index] == col) return data[index];
    }
    throw std::logic_error("Could not find row and colum of matrix: " + std::to_string(row) + " " +
                           std::to_string(col));
}
}